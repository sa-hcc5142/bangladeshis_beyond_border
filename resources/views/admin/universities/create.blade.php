...
@section('header', 'Add New University')

@section('content')
<div class="bg-white shadow rounded-lg">
    <form action="{{ route('admin.universities.store') }}" method="POST" class="p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="mb-4">
                ...
                <select name="region" id="region" required
                    ...
                    <option value="Latin America" {{ old('region') == 'Latin America' ? 'selected' : '' }}>Latin America</option>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4" required
                class="mt-1 px-3 py-2 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border border-gray-300 rounded-md">{{ old('description') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
